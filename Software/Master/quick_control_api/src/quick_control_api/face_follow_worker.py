...
DEFAULT_PAN_GAIN_DEG_PER_NORM = 4.8  # 水平轴增益，略降以减少中心附近过冲
DEFAULT_TILT_GAIN_DEG_PER_NORM = 6.0  # 俯仰轴增益，略降以减少提前刹停前的摆动
DEFAULT_PAN_DEAD_ZONE_NORM = 0.02  # 水平轴死区，缩小后更容易贴近目标中心
DEFAULT_TILT_DEAD_ZONE_NORM = 0.025  # 俯仰轴死区，保留少量容忍避免抖动
DEFAULT_PAN_RESUME_ZONE_NORM = 0.03  # 水平轴恢复区，减小交叉过零后提前 hold
DEFAULT_TILT_RESUME_ZONE_NORM = 0.035  # 俯仰轴恢复区，减小交叉过零后提前 hold
DEFAULT_MIN_PAN_STEP_DEG = 0.35  # 水平轴最小步进角，避免靠近中心时被最小步长顶过头
DEFAULT_MIN_TILT_STEP_DEG = 0.7  # 俯仰轴最小步进角，兼顾贴近目标和克服负载
DEFAULT_PAN_MIN_STEP_ZONE_NORM = 0.08  # 水平轴仅在偏差更明显时才启用最小步进补偿
DEFAULT_TILT_MIN_STEP_ZONE_NORM = 0.12  # 俯仰轴仅在偏差更明显时才启用最小步进补偿
DEFAULT_MAX_PAN_STEP_DEG = 1.6  # 水平轴单次最大步进角，限制大幅甩动
DEFAULT_MAX_TILT_STEP_DEG = 1.8  # 俯仰轴单次最大步进角，限制大幅甩动
...
DEFAULT_STICTION_EPS_DEG = 0.18  # 更早把小幅真实位移视作已动，避免误触 breakaway
DEFAULT_STICTION_FRAMES = 3  # 延后 breakaway 触发，给小步进更多收敛机会
DEFAULT_PAN_BREAKAWAY_STEP_DEG = 1.2  # 水平轴默认破静摩擦补偿步进角
DEFAULT_PAN_BREAKAWAY_STEP_POS_DEG = 1.0  # 水平轴正方向破静摩擦补偿步进角
DEFAULT_PAN_BREAKAWAY_STEP_NEG_DEG = 2.0  # 水平轴负方向破静摩擦补偿步进角
DEFAULT_PAN_NEGATIVE_SCALE = 1.45  # 保留一定负方向补偿，但避免过度放大
DEFAULT_TILT_BREAKAWAY_STEP_DEG = 1.4  # 俯仰轴破静摩擦补偿步进角
