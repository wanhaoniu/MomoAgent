...
| 控制服务层 | 把机械臂能力包装成 API、行为模式 | `Software/Master/momo_robot_service/` | 这是“控制中台” |
...
graph LR
    ...
    C["HTTP API<br/>momo_robot_service"] --> B
    D["自然语言/语音<br/>momo_agent"] --> E["Nanobot agent 技能链路"]
    E --> C
...
- `momo_robot_service` 把“连机械臂、读状态、关节微调、回 home、跟随、巡航”等能力暴露成 HTTP 接口。
- `momo_agent` 本身不直接碰硬件，它更像“会说话的入口”，把自然语言交给 Nanobot agent，再由技能链路去调用 `momo_robot_service`。
- `face_loc` 负责输出目标在画面中的位置；`momo_robot_service` 的 follow worker 会读取它的 `/latest` 结果来驱动机械臂跟随。
...
3. `Software/Master/momo_robot_service/src/momo_robot_service/service.py`
...
### 4.3 `momo_robot_service/service.py`
...
source .venv/bin/activate
python Software/Master/momo_robot_service/main.py --host 0.0.0.0 --port 8010
```
...
1. `face_loc` 先做检测
2. 它通过 `/latest` 提供目标位置
3. `momo_robot_service` 的 follow worker 轮询这个结果
4. 然后把偏差换成一个很小的关节修正量
...
```text
请先阅读这个仓库里的 momo_robot_service/service.py、sdk/runtime_compat.py 和 sdk/real_arm.py，
用 0 基础能听懂的话解释：
...
```text
请只修改 momo_robot_service 这一层，不要动 GUI 和 SDK。
...
- API：`Software/Master/momo_robot_service/main.py`
...
3. **最适合新手理解控制链路的入口是 `momo_robot_service`。**
...
1. 先看这份文档，建立全局地图
2. 再看 `Software/Master/momo_robot_service/README.md`
